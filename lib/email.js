...
function getEmailTemplate(type, data = {}) {
  const templates = {
    ...
    [EmailTypes.REQUEST_CANCELLED]: {
      subject: 'Your Song Request Has Been Cancelled',
      to: (data) => data.email,
      from: process.env.EMAIL_FROM,
      html: (data) => `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #333;">
          <h2 style="color: #3B82F6;">Song Request Cancelled</h2>
          <p>Hello ${data.userName},</p>
          <p>Your song request has been successfully cancelled as requested:</p>
          ...
          <div style="background-color: #f8f9fa; border-left: 4px solid #3B82F6; padding: 15px; margin: 15px 0;">
            ...
            <p><strong>Amount Refunded:</strong> ${data.formattedAmount}</p>
            <p><strong>Date:</strong> ${data.cancelDate}</p>
          </div>
          
          <p>The refund amount has been credited back to your wallet balance and is available for immediate use.</p>
          
          <p>If you did not initiate this cancellation or have any questions, please contact our support team.</p>
          
          <p>Thank you for using our service!</p>
          
          <div style="margin-top: 30px; font-size: 12px; color: #666; border-top: 1px solid #eee; padding-top: 15px;">
            <p>This is an automated message. Please do not reply to this email.</p>
            <p>Reference ID: ${data.requestId}</p>
          </div>
        </div>
      `,
      text: (data) => `
        Song Request Cancelled
        
        Hello ${data.userName},
        
        Your song request has been successfully cancelled as requested:
        
        Song: ${data.songTitle}
        Amount Refunded: ${data.formattedAmount}
        Date: ${data.cancelDate}
        
        The refund amount has been credited back to your wallet balance and is available for immediate use.
        
        If you did not initiate this cancellation or have any questions, please contact our support team.
...
const getPaymentStatusTemplate = (data) => {
  // Ensure status is a string and has a value before calling toLowerCase()
