...
// Choose which email provider to use - get from environment
const EMAIL_PROVIDER = process.env.EMAIL_PROVIDER || 'smtp';

// Only import SendGrid if we're using it
let sgMail = null;
if (EMAIL_PROVIDER === 'sendgrid') {
  try {
    ...
    console.warn('SendGrid package not installed. If you want to use SendGrid, please install @sendgrid/mail');
  }
}
...
function getEmailTemplate(type, data = {}) {
  const templates = {
    ...
    [EmailTypes.PASSWORD_RESET]: {
      ...
      text: `Hello ${data.userName || 'there'},\n\nYou requested a password reset. Please click the link below to reset your password:\n\n${data.resetLink || 'https://example.com/reset-password'}\n\nIf you didn't request this, please ignore this email.\n\nBest Regards,\nThe SongBid Team`,
      html: `<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
               <h2 style="color: #3b82f6;">Password Reset Request</h2>
               <p>Hello ${data.userName || 'there'},</p>
               <p>You requested a password reset. Please click the button below to reset your password:</p>
               <div style="text-align: center; margin: 20px 0;">
                 <a href="${data.resetLink || 'https://example.com/reset-password'}" style="display: inline-block; background-color: #3b82f6; color: white; padding: 10px 20px; text-decoration: none; border-radius: 4px;">Reset Password</a>
               </div>
  ...
  return templates[type] || {
    ...
    html: '<div style="font-family: Arial, sans-serif;">This is a notification from SongBid.</div>'
  };
}
...
export async function sendEmail({ to, subject, text, html, from }) {
  ...
  try {
    if (EMAIL_PROVIDER === 'sendgrid') {
      if (!sgMail) {
        throw new Error('SendGrid is not configured. Please install @sendgrid/mail or use SMTP.');
      }
